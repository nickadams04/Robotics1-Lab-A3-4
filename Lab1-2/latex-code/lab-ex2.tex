...
Στην προηγούμενη ενότητα παρατηρήσαμε ότι ο γραμμικός ελεγκτής PD, παρότι κατάφερε να
σταθεροποιήσει το σύστημα, παρουσίασε σημαντικό σφάλμα μόνιμης κατάστασης \(e_{ss}\), ακόμα και για τις
βέλτιστες τιμές κερδών. Αυτό οδήγησε στην ανάγκη εισαγωγής της αντιστάθμισης βαρύτητας.

Η αντιστάθμιση βαρύτητας (gravity compensation) είναι η τεχνική κατά την οποία προσθέτουμε στον νόμο
ελέγχου έναν υπολογισμένο όρο, ο οποίος ισούται με τη ροπή που ασκεί η βαρύτητα στους συνδέσμους
του ρομπότ στην τρέχουσα θέση τους. Είναι απαραίτητη, γιατί ένας απλός ελεγκτής PD παράγει ροπή μόνο όταν υπάρχει
σφάλμα θέσης. Επειδή η βαρύτητα ασκεί συνεχή δύναμη στο Pendubot (προσπαθώντας να το κατεβάσει), ο ελεγκτής PD
αναγκάζεται να διατηρεί ένα μόνιμο σφάλμα ώστε να παράγει την απαιτούμενη ροπή συγκράτησης.

Εισάγοντας τον όρο αντιστάθμισης \(g(q)\) που υπολογίσαμε στη θεωρητική ανάλυση, παίρνουμε τον νέο
νόμο ελέγχου
\[
u(t) = K_p\,e(t) + K_d\,\dot e(t) + g(q).
\]
Για τα βέλτιστα κέρδη \(K_p, K_d\) που βρήκαμε στο προηγούμενο κομμάτι, δηλαδή \((K_p,K_d)=(20,\,0.95)\),
παίρνουμε τις τιμές
\[
e_{ss} = -0.67^\circ,\quad t_r = 0.42\,\text{s},\quad M_p = 0.0139\,\text{rad},
\]
και το παρακάτω διάγραμμα:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{gravcomp_20_0.95.pdf}
    \caption{Απόκριση του Pendubot με PD έλεγχο και αντιστάθμιση βαρύτητας για \(K_p = 20\), \(K_d = 0.95\).}
    \label{fig:gravcomp-20-095}
\end{figure}

Φαίνεται λοιπόν πως πετύχαμε σημαντική βελτίωση στο σφάλμα μόνιμης κατάστασης, ενώ ο χρόνος ανύψωσης και το
overshoot έμειναν πρακτικά αμετάβλητα. Να σημειωθεί πως το ρομπότ σταθεροποιήθηκε σε θέση
ελαφρώς ψηλότερη από την επιθυμητή και αυτό μπορεί να συμβαίνει είτε λόγω σφάλματος στον υπολογισμό
των σταθερών \(\varphi_1, \varphi_2\) είτε λόγω της στατικής τριβής του μηχανισμού.

Το παραπάνω σφάλμα επαληθεύεται θεωρώντας ως είσοδο ελέγχου μόνο τον όρο αντιστάθμισης, δηλαδή \(u = g_1\).
Το Pendubot παραμένει αρχικά στη φυσική του θέση και, τοποθετώντας κατόπιν χειροκίνητα τον σύνδεσμο
σε μία γωνία \(\theta\), η θεωρητική προσδοκία είναι η διατήρηση της θέσης αυτής λόγω του όρου
αντιστάθμισης βαρύτητας. Ωστόσο, επειδή ο όρος \(g_1\) δεν περιγράφει το μοντέλο με απόλυτη ακρίβεια,
παρατηρήθηκε μικρή απόκλιση της γωνίας, με το σύστημα να ισορροπεί τελικά σε μία νέα θέση
\(\theta' > \theta\). Επιπλέον, παρατηρήθηκε ότι καθώς αυξανόταν η τιμή της αρχικής γωνίας \(\theta\),
το Pendubot αδυνατούσε να ισορροπήσει, οδηγώντας το σύστημα σε αστάθεια.
...
Κατά την διεξαγωγή του πειράματος η επιθυμητή μεσαία θέση επιτεύχθηκε με επιτυχία σε μικρό χρονικό διάστημα και με λιγοστές αλλαγές επιτάχυνσης από τον μη γραμμικό ελεγκτή δύο φάσεων. Επιπλέον, για μικρές μεταβολές της θέσης μετά την ισορροπία, η φάση εξισορρόπησης, δηλαδή ο δεύτερος ελεγκτής,  εξασφαλίζει την επαναφορά στην επιθυμητή θέση. Επομένως, διαπιστώνεται επιτυχής λειτουργία του ελεγκτή και στα δύο στάδια.
